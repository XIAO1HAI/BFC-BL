...
class iFSLModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.weak:
            ...
            loss = self.compute_seg_objective(pred_seg, batch['query_mask'])
    ...
    def compute_seg_objective(self, pred_seg, gt_mask):
        ''' supports 1-way training '''
        with torch.no_grad():
            #     print(gt_mask.shape)
            gt_1 = class2one_hot(gt_mask, 2)
            gt_1 = gt_1[0].cpu().numpy()
            gt_2 = one_hot2dist(gt_1)
            gt = torch.tensor(gt_2).unsqueeze(0).cuda()
