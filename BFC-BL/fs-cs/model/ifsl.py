...
# from .shape_loss import DistBinaryDiceLoss
...
class iFSLModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.weak:
            ...
            loss = a * self.boundary_seg_loss(pred_seg, batch['query_mask']) + (1 - a) * self.region_seg_nllloss(logit_seg, batch['query_mask'])
    ...
    def collect_class_presence(self, logit_mask):
        ...
        # print(logit_mask[:, :, 1].max(dim=-1)[0].max(dim=-1)[0])
        # print(math.log(0.5))
        ...
        # print(class_activation.type(logit_mask.dtype))
    ...
    def region_seg_nllloss(self, logit_mask, gt_mask):
        return F.nll_loss(logit_mask, gt_mask.long()) 
    def boundary_seg_loss(self, pred_seg, gt_mask):
        ''' supports 1-way training '''
        # print(gt_mask)
        # loss = DistBinaryDiceLoss()
        # loss = loss(logit_mask, gt_mask)
        ###################
        with torch.no_grad():
            #     print(gt_mask.shape)
            gt_1 = class2one_hot(gt_mask, 2)
            # print(gt_1.shape)
            # print(gt_1)
            gt_1 = gt_1.cpu().numpy()
            # print(gt_1[0])
            # print(gt_1.shape)
            gt_2 = one_hot2dist(gt_1)
            gt = torch.tensor(gt_2).cuda()
            # print(gt)
            # gt = gt.view(self.batch, *gt.shape[0: ])
            # print(gt.shape)
            # print(pred_seg.shape)

        logits = class2one_hot(pred_seg, 2)
        # print(logits.shape)
        
        loss = SurfaceLoss()
        res = loss(logits, gt, None)
        return res
        ###################
        # return F.nll_loss(logit_mask, gt_mask.long())
    def compute_cls_objective(self, shared_masks, gt_presence):
        ...
        # print(prob_avg)
        # print(gt_presence.long().squeeze(-1))
        return F.nll_loss(prob_avg, gt_presence.long().squeeze(-1))

    def merge_bg_masks(self, shared_fg_masks):# (1,1,2,100,100)
        # B, N, H, W
        logit_fg = shared_fg_masks[:, :, 1]# (1,1,100,100)
        # B, 1, H, W
        logit_episodic_bg = shared_fg_masks[:, :, 0].mean(dim=1)# (1,100,100)
        # B, (1 + N), H, W
        logit_mask = torch.cat((logit_episodic_bg.unsqueeze(1), logit_fg), dim=1)# (1,2,100,100)
